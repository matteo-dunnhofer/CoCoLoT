...
import time
import os
#sys.path.append('/media/TBData2/kristian_projects/conda_env/super_dimp/DiMP_LTMU/Stark/')
sys.path.append(vot_path.base_path + 'mlpLT/Stark/')
...
class stark_vot20_lt(object):
    ...
    def track(self, img_rgb, detect=False):
        # track
        outputs = self.tracker.track(img_rgb, detect=detect)
        pred_bbox = outputs['target_bbox']
        conf_score = outputs["conf_score"]
        return pred_bbox, conf_score

    def track2(self, img_rgb, detect=False):
        # track
        outputs = self.tracker.track2(img_rgb, detect=detect)
        pred_bbox = outputs['target_bbox']
        conf_score = outputs["conf_score"]
        return pred_bbox, conf_score

    def update2(self, img_rgb, state, conf_score):
        # update
        outputs = self.tracker.update2(img_rgb, state, conf_score)
