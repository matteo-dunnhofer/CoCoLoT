...
class STARK_ST(BaseTracker):
    ...
    def track(self, image, info: dict = None, detect=False):
        H, W, _ = image.shape
        if not detect:
            self.frame_id += 1
        ...
        #conf_score = (out_dict["pred_logits"].view(-1) * 0.9).sigmoid().item()
        # update template
    ...
    def track2(self, image, info: dict = None, detect=False):
        H, W, _ = image.shape
        if not detect:
            self.frame_id += 1
        # get the t-th search region
        ...
        return {"target_bbox": self.state,
                    "conf_score": conf_score}
    """
    def update2(self, image, state, conf_score):
        # update template
        for idx, update_i in enumerate(self.update_intervals):
            if self.frame_id % update_i == 0 and conf_score > 0.5:
    ...
    def update2(self, image, state, conf_score):
        # update template
        #if self.frame_id == self.update_idx and conf_score > 0.5:
        if self.frame_id == self.update_idx and conf_score > 3:
            z_patch_arr, _, z_amask_arr = sample_target(image, state, self.params.template_factor,
                                                            output_sz=self.params.template_size)  # (x1, y1, w, h)
            template_t = self.preprocessor.process(z_patch_arr, z_amask_arr)
            with torch.no_grad():
            ...
            #self.update_idx += self.update_intervals[0]
            self.update_idx += (self.update_intervals[0] - self.update_idx % self.update_intervals[0])
        #elif self.frame_id == self.update_idx and conf_score <= 0.5:
        elif self.frame_id == self.update_idx and conf_score <= 3:
            self.update_idx += 1
        else:
            return
